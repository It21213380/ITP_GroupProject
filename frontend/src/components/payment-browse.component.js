...
const PaymentBrowse = () => {
    ...
    const [spaceData, setPaymentData] = useState({})
    ...
    useEffect(() => {
        const getPaymentData = async () => {
            setDataReady(false)
            await axios.get('http://localhost:5000/api/payment/')
                .then(response => {
            ...
            list?.map(entry =>
                <div className='border p-2 row'>
                    <div className='col-md'>
                        <Link to={`/payment/${entry._id}`} className='text-decoration-none '>
                        ...
                        <ul>
                            <li>
                                <p><i alt='Profile' className='bi bi-people-fill img-fluid fs-3 align-middle' />
                                    Payment for {entry?.peopleCount} {entry?.peopleCount == 1 ? "person" : "people"}
                                </p>
                            </li>
                            <li>
    ...
    function renderList() {
        if (dataReady) {
            if (paymentData && Array.isArray(paymentData)) {
                setListRender(
                    search == ""
                        ? paymentListRender(paymentData)
                        : paymentListRender(
                            paymentData?.filter(entry =>
